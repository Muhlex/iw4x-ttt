...
handleNormalDeath( lifeId, attacker, eInflictor, sWeapon, sMeansOfDeath )
{
	...
	attacker incPersStat( "kills", !level.ttt.enabled );
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( !isFauxDeath )
	{
		...
		if (!level.ttt.enabled) victim.statusicon = "hud_status_dead";
	}
	...
	if ( !isDefined( victim.switching_teams ) )
	{
		...
		victim incPersStat( "deaths", !level.ttt.enabled );
	...
	// obituary
	if (!level.ttt.enabled) obituary( victim, attacker, sWeapon, sMeansOfDeath );
	...
	if ( victim isOnLadder() || victim isMantling() || !victim isOnGround() || isDefined( victim.nuked ) )
	{
		...
		victim notify("start_ragdoll");
	}

	if ( !isDefined( victim.switching_teams ) )
		thread maps\mp\gametypes\_deathicons::addDeathicon( victim.body, victim, victim.team, 5.0 );

	victim thread delayStartRagdoll( victim.body, sHitLoc, vDir, sWeapon, eInflictor, sMeansOfDeath );
	...
	level.lastDeath = spawnstruct();
	level.lastDeath.victim = victim;
	level.lastDeath.attacker = attacker;
	level.lastDeath.attackerNum = attackerNum;
	level.lastDeath.killcamentityindex = killcamentityindex;
	level.lastDeath.killcamentitystarttime = killcamentitystarttime;
	level.lastDeath.sWeapon = sWeapon;
	level.lastDeath.deathTimeOffset = deathTimeOffset;
	level.lastDeath.psOffsetTime = psOffsetTime;
